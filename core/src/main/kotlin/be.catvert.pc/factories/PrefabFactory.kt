...
enum class PrefabFactory(val prefab: Prefab) {
    ...
    Sprite(
            Prefab("sprite", "Catvert", GameObject(GameObject.Tag.Sprite, Rect(size = Size(50, 50)), null) {
                this += AtlasComponent(componentDataOf(0, AtlasComponent.AtlasData("default", 0f, Constants.defaultAtlasPath.first.toFileWrapper() to Constants.defaultAtlasPath.second)))
            })
    ),
    PhysicsSprite(
    ...
    Player(
            Prefab("player", "Catvert", GameObject(GameObject.Tag.Player, Rect(size = Size(48, 98)), null) {
                ...
                this += AtlasComponent(componentDataOf(0, AtlasComponent.AtlasData("default", 0f, Constants.atlasDirPath.child("More Enemies Animations/aliens.atlas").toFileWrapper() to "alienGreen_stand")))

                this += PhysicsComponent(false, 10, MovementType.SMOOTH, jumpHeight = 200, onJumpAction = SoundAction(jumpSoundIndex))
    ...
    Spider(
            Prefab("spider", "Catvert", GameObject(GameObject.Tag.Enemy, Rect(size = Size(48, 48)), null) {
                this += AtlasComponent(componentDataOf(0, AtlasComponent.AtlasData.fromAnimationRegion("walk", Constants.atlasDirPath.child("More Enemies Animations/enemies.atlas").toFileWrapper(), "spider_walk", 0.33f)))
    ...
    SnakeSlime(
            Prefab("snake slime", "Catvert", GameObject(GameObject.Tag.Enemy, Rect(size = Size(35, 120)), null) {
                this += AtlasComponent(componentDataOf(0, AtlasComponent.AtlasData.fromAnimationRegion("base", Constants.atlasDirPath.child("More Enemies Animations/enemies.atlas").toFileWrapper(), "snakeSlime", 0.33f)))
                this += PhysicsComponent(true)
                this += LifeComponent(RemoveGOAction())
    ...
    Bee(
            Prefab("bee", "Catvert", GameObject(GameObject.Tag.Enemy, Rect(size = Size(35, 35)), null) {
                this += AtlasComponent(componentDataOf(0, AtlasComponent.AtlasData("default", 0f, Constants.atlasDirPath.child("More Enemies Animations/enemies.atlas").toFileWrapper() to "bee")))
    ...
    GoldCoin(
            Prefab("gold coin", "Catvert", GameObject(GameObject.Tag.Special, Rect(size = Size(35, 35)), null) {
                this += AtlasComponent(componentDataOf(0, AtlasComponent.AtlasData("default", 0f, Constants.atlasDirPath.child("Jumper Pack/spritesheet_jumper.atlas").toFileWrapper() to "coin_gold")))
                this += PhysicsComponent(true)
