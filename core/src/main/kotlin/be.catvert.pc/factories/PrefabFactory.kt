...
object PrefabSetup {
    fun setupSprite(region: AtlasRegion) = Prefab("sprite", GameObject(Tags.Sprite.tag, box = Rect(size = Size(50, 50))) {
        this += AtlasComponent(0, AtlasComponent.AtlasData("default", region))
    })

    fun setupPhysicsSprite(region: AtlasRegion) = Prefab("physics sprite", GameObject(Tags.Sprite.tag, box = Rect(size = Size(50, 50))) {
        this += AtlasComponent(0, AtlasComponent.AtlasData("default", region))
...
enum class PrefabFactory(val type: PrefabType, val prefab: Prefab) {
    ...
    SnakeSlime_Kenney(PrefabType.Kenney,
            Prefab("snake slime", GameObject(Tags.Enemy.tag, box = Rect(size = Size(35, 120))) {
                this += AtlasComponent(0, AtlasComponent.AtlasData("base", Constants.packsKenneyDirPath.child("enemies.atlas").toFileWrapper(), "snakeSlime", 0.33f))
    ...
    Furball_SMC(PrefabType.SMC,
            Prefab("furball", GameObject(Tags.Enemy.tag, box = Rect(size = Size(48, 48))) {
