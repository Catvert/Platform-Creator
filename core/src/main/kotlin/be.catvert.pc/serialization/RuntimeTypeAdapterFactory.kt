...
`</pre> *
 */
class RuntimeTypeAdapterFactory<T> private constructor(private val baseType: Class<*>?, private val typeFieldName: String?) : TypeAdapterFactory {
    ...
    override fun <R : Any> create(gson: Gson, type: TypeToken<R>): TypeAdapter<R>? {
        if (baseType == null || !baseType.isAssignableFrom(type.rawType)) {
            return null
        }
        ...
        return object : TypeAdapter<R>() {
            ...
            override fun read(`in`: JsonReader): R {
                ...
                val labelJsonElement = jsonElement.asJsonObject.remove(typeFieldName) ?: throw JsonParseException("cannot deserialize " + baseType
                        + " because it does not define a field named " + typeFieldName)
                val label = labelJsonElement.asString
                val delegate = labelToDelegate[label] as TypeAdapter<R> ?: throw JsonParseException("cannot deserialize " + baseType + " subtype named "
