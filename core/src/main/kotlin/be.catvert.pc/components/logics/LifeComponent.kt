        ...
        override fun insertImgui(labelName: String, gameObject: GameObject, level: Level) {
            with(ImGui) {
                if (treeNode(labelName)) {
                    ImguiHelper.addImguiWidget("start", this@LifePointActions::onStartAction, gameObject, level, ExposeEditorFactory.empty)
                    ImguiHelper.addImguiWidget("end", this@LifePointActions::onEndAction, gameObject, level, ExposeEditorFactory.empty)
                    treePop()
                }
            }
        }
